...
public:
 ...
 int evalRule(const Rule& rule) {
    ...
    // Track variable mappings and positions
    map<string, vector<pair<size_t, size_t>>> variablePositions;
    
    // First pass: Collect variable information across body predicates
    for (size_t predIndex = 0; predIndex < bodyPredicates.size(); predIndex++) {
        const Predicate& predicate = bodyPredicates[predIndex];
        ...
        const auto& parameters = predicate.getParameters();
        for (size_t paramIndex = 0; paramIndex < parameters.size(); paramIndex++) {
            const Parameter& param = parameters[paramIndex];
            if (param.getIsID()) {
                variablePositions[param.getValue()].push_back({predIndex, paramIndex});
            }
        }
    }
    ...
    for (const Predicate& bodyPredicate : bodyPredicates) {
        ...
        // Prepare projection and variable mapping
        vector<size_t> projectIndices;
        vector<string> variableNames;

        // Determine which variables to project based on head predicate
        for (size_t i = 0; i < parameters.size(); i++) {
            if (parameters[i].getIsID()) {
                string varName = parameters[i].getValue();
                
                // Check if this variable is in the head predicate
                for (const Parameter& headParam : headPredicate.getParameters()) {
                    if (headParam.getValue() == varName) {
    ...
    if (!intermediateResults.empty()) {
        ...
        // Final projection to match head predicate
