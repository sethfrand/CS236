...
string evalRules() 
{
    ...
    do{
        ...
        while (tuplesAdded)
        {
            ss << endl << "Schemes populated after " << numPasses << "passes through the Rules" << endl << end;
            return ss.str();
        }
    }
}   
...
int evalRule(const Rule& rule)
{
    ...
    for (size_t =0; i< parameters.size(); i++)
    {
        const Parameters& param = parameters[i];
        ...
        if (!param.getIsID())
        {
            ...
            if (variablePosition.count(varName))
    ...
    vector<size_t> projectIndicies;
    for (const auto& pair : variablePositions)
    {
        projectIndicies.push_back(pair.second);
    }

    Relation projected = result.project(projectIndicies);
...
vector<size_t> headPreojectIndices;
vector<size_t> headVariableNames;

const auto& headParameters = headPredicate.getParameters()
for (const Parameters& param : headParameters)
{
    string varName = param.getValue();
    ...
    for (size_t = 0; i < joinedScheme.size(); i++)
    {
    ...
    headProjectIndicies.push_back(index);
}

Relation projected = joinedResult.project(headProjectIndicies);
