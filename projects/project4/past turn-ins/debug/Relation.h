...
class Relation {
 ...
 public:
  ...
  Relation join (const Relation& right)
  {
    ...
    Relation result(left.getName() + "-" + right.getName(), combinedScheme);
...
{
    // Ensure the relations have the same scheme
    if (scheme != other.scheme) {
        throw runtime_error("Cannot union relations with different schemes");
    }

    // Create a new relation with the same name and scheme
    Relation result(name, scheme);

    // Add all tuples from this relation
    for (const auto& tuple : tuples) 
    {
        result.addTuple(tuple);
    }

    // Add tuples from the other relation, 
    // automatically avoiding duplicates due to set's nature
