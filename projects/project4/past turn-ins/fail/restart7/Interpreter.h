...
string evalRules() {
    ...
    do {
        ...
        cout << "Pass " << numPasses << " starting..." << endl;  // Debug print
        ...
        for (const Rule& rule : rules) {
            ...
            cout << "Rule " << rule.toString() << " added " << tupleAddedByRule << " tuples" << endl;  // Debug print
...
int evalRule(const Rule& rule) {
    ...
    for (const Predicate& bodyPredicate : bodyPredicates) {
        ...
        if (!database.hasRelation(relationName)) {
            cout << "Skipping predicate: No relation named " << relationName << endl;
            continue;
        }
        ...
        for (size_t i = 0; i < parameters.size(); i++) {
            ...
            if (!param.getIsID()) {
                ...
                if (variablePositions.count(varName)) {
                    // If variable already seen, enforce consistency
        ...
        cout << "Intermediate relation for " << relationName 
             << ": " << result.getTuples().size() << " tuples" << endl;
    ...
    // Debug: print joined result details
    cout << "Joined result: " << joinedResult.getTuples().size() << " tuples" << endl;
    ...
    cout << "Tuples added to " << headName << ": " << tuplesAdded << endl;
