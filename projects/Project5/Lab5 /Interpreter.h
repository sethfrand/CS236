   ...
   static Graph makeGraph(const std::vector<Rule>& rules)
{
    ...
    for (size_t i = 0; i < rules.size(); i++) 
    {
        ...
        for (const Predicate& bodyPredicate : fromRule.getBodyPredicates()) 
        {
            ...
            for (size_t j = 0; j < rules.size(); j++) 
            {
                ...
                if (bodyPredicate.toString() == toRule.getHeadPredicate().toString()) 
                {
                    // Add edge from i (fromRule) to j (toRule)
                    graph.addEdge(i, j);

                    //std::cout << "dependency found: (" << i << "," << j << ")" << std::endl;
                }
            }
        }
    }
    ...
    for (const auto& pair : graph.getNodes())  
    {
        ...
        // Only print edges that actually exist (skip printing nodes without edges)
        std::string edges = node.toString();
        if (!edges.empty()) 
        {
            std::cout << edges;
        }
