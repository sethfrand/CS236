...
public:
    ...
    void evalRule(const Rule& rule) {
        ...
        for (const Predicate& bodyPredicate : rule.getBodyPredicates()) {
            ...
            Relation result = database.getRelation(bodyPredicate.getName());
            result = applySelectionsAndProjections(result, bodyPredicate);
        ...
        insertResultsIntoDatabase(joinedResult, rule.getHeadPredicate());
    }
    ...
    void insertResultsIntoDatabase(Relation result, const Predicate& headPredicate) {
        ...
        Relation& targetRelation = database.getRelation(relationName);
        int beforeSize = targetRelation.getTuples().size();
        for (const Tuple& tuple : result.getTuples()) {
