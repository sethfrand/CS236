...
public:
    ...
    void evalRule(const Rule& rule) {
        ...
        for (const Predicate& bodyPredicate : rule.getBodyPredicates()) {
            ...
            // Apply selections and projections
        ...
        for (const Parameter& param : headParams) {
            ...
            if (!found) {
                ...
                // Instead of returning early, we'll continue with other variables
            }
        }
        
        // Only proceed if we have valid projection indices
        if (projectIndices.size() == headParams.size()) {
            // Project and rename
            Relation projectedResult = result.project(projectIndices);
            Relation finalResult = projectedResult.rename(Scheme(headVarNames));
            ...
            cout << "Warning: Not all head variables found in result scheme" << endl;
        }
    }
    ...
    Relation applySelectionsAndProjections(Relation relation, const Predicate& predicate) {
        try {
            ...
            // First apply selections for constants and track variables
            for (size_t i = 0; i < predicate.getParameters().size(); i++) {
                const Parameter& param = predicate.getParameters()[i];
                
                if (!param.getIsID()) {
                    // For constants, apply selection
                    relation = relation.select(i, param.getValue());
                } else {
                    // For variables
                    string varName = param.getValue();
                    if (variablePositions.count(varName) > 0) {
                        // Repeated variable, apply selection for matching columns
                        relation = relation.select(variablePositions[varName], i);
                    } else {
                        // First time seeing this variable, track its position
            ...
            // Only do projection and renaming if there are variables
