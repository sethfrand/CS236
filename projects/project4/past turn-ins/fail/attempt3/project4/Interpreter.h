...
string evalRules() 
{
    ...
    do {
        ...
        for (const Rule& rule: rules)
        {
            ...
            int tupleAddedByRule = evalRule(rule, resultRelation);
            ...
            // Add output for each tuple in the result relation
            if (tupleAddedByRule > 0) {
                for (const Tuple& tuple : resultRelation.getTuples()) {
                    ss << "  ";
                    for (size_t i = 0; i < resultRelation.getScheme().size(); i++) {
                        ss << resultRelation.getScheme()[i] << "='" << tuple[i] << "'";
                        if (i < resultRelation.getScheme().size() - 1) {
                            ss << ", ";
                        }
                    }
                    ss << endl;
