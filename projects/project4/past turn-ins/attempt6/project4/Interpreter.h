...
public:
    ...
    void evalFacts() {
        ...
        for (const Predicate& factPredicate : facts) {
            ...
            if (values.size() != relation.getScheme().size()) {
                cerr << "Error: Tuple size mismatch for relation '" << relationName << "'" << endl;
                continue;
            }
    ...
    // Process each body predicate to create intermediate results
    ...
    const auto& headParams = headPredicate.getParameters();

    for (const Parameter& param : headParams) {
        for (size_t j = 0; j < joinedResult.getScheme().size(); j++) {
            if (joinedResult.getScheme()[j] == param.getValue()) {
                headProjectIndices.push_back(j);
                headAttributes.push_back(param.getValue());
                break;
            }
        }
    }

    // Apply projection on joinedResult to only keep relevant columns
    Relation projectedResult = joinedResult.project(headProjectIndices);
    projectedResult = projectedResult.rename(Scheme(headAttributes));

    // Insert only the correctly projected tuples into the database
    Relation& targetRelation = database.getRelation(headName);
    int tuplesAdded = 0;
    
    for (const Tuple& tuple : projectedResult.getTuples()) {
        ...
        if (afterSize > beforeSize) {
            tuplesAdded++;
            cout << "  ";
            for (size_t i = 0; i < targetRelation.getScheme().size(); i++) {
                cout << targetRelation.getScheme()[i] << "='" << tuple[i] << "'";
                if (i < targetRelation.getScheme().size() - 1) {
                    cout << ", ";
                }
            }
            cout << endl;
